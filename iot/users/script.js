...
const map = L.map('map').setView([14.4497, 120.9826], 16); // Zoom level adjusted for better view
...
function fetchAddress(lat, lon, index) {
    ...
    fetch(apiURL)
        .then(response => response.json())
        .then(data => {
            let address = data.display_name || 'Address not found'; // Get the formatted address
